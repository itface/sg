...
@Entity
@Table(name="knb1_log")
public class Knb1Log {
	private static final long serialVersionUID = -129L;
	@Id
	@GeneratedValue(strategy=GenerationType.SEQUENCE,generator="seq_knb1_log")
	@SequenceGenerator(name="seq_knb1_log",sequenceName="seq_knb1_log",allocationSize=1)
	private long id;
	...
	/*
	public Knb1Log(long sapclient,Knb1 sknb1,Knb1 gknb1) throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException{
		...
		if(sknb1!=null){
			...
			for(int j=0;j<fields.length;j++){
				...
				if(!name.equals("serialVersionUID")&&!name.equals("id")&&!name.equals("sapclient")){
					...
					Method method = Knb1Log.class.getMethod(methodName, new Class[]{fieldType});
	...
	*/
	public Knb1Log(Knb1 knb1,long sapclient,String optmsg,String opt,String optflag,String opttype,String optuser,Date opttime) throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException{
		this.sapclient=sapclient;
		this.opt=opt;
		...
		if(knb1!=null){
			Class modelClass = knb1.getClass();
			Field[] fields = modelClass.getDeclaredFields();
			for(int j=0;j<fields.length;j++){
				String name = fields[j].getName();
				if(!name.equals("serialVersionUID")&&!name.equals("id")&&!name.equals("sapclient")&&!name.equals("garden_flag")&&!name.equals("datakey")){
					...
					String getMethodName = "get"+name.substring(0, 1).toUpperCase()+name.substring(1);
					Method getMethod = modelClass.getMethod(getMethodName, new Class[]{});
					Object value = getMethod.invoke(knb1, new Object[]{});
